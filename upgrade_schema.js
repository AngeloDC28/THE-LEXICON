...
const newArchiveData = archiveData.map(collection => {
  const cData = collectionsConfig[collection.id] || collectionsConfig['mcqueen-ss99'];
  
  // Evolve the 'images' array from string URLs to full objects
  const updatedImages = collection.images.map((imgSrc, idx) => {
    let imgPath = typeof imgSrc === 'string' ? imgSrc : imgSrc.src;
    
    const numHotspots = 3;
    const offset = idx % 4;
    const posSet = positionsPool[idx % positionsPool.length];
    
    let generatedHotspots = [];
  ...
  return {
    ...
    hotspots: [] // clear the root level hotspots as they are now per image
  };
});

const outputCode = 'const archiveData = ' + JSON.stringify(newArchiveData, null, 2) + ';\n\nif (typeof module !== "undefined") {\n  module.exports = archiveData;\n}\n';

fs.writeFileSync('database.js', outputCode, 'utf-8');
console.log('Successfully upgraded database schema to support per-image hotspots.');
