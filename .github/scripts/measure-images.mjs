 ...
 * Idempotent: output is stable across machines and only changes when image
 * dimensions change. Skips files it can't parse with a warning.
 */
...
for (const filePath of files) {
  ...
  try {
    ...
    if (existing[rel] && existing[rel].w === d.w && existing[rel].h === d.h) {
      skipped++;
      continue;
    }
    existing[rel] = { w: d.w, h: d.h };
    measured++;
  } catch (e) {
...
const normalized = {};
for (const [k, v] of Object.entries(existing)) normalized[k] = { w: v.w, h: v.h };

writeFileSync(OUT, JSON.stringify(normalized, null, 2) + '\n', 'utf8');

// Also emit a JS module for browser import.
const browserDims = {};
for (const [k, v] of Object.entries(normalized)) browserDims[k] = { w: v.w, h: v.h };
...
console.log(`LEXICON_MEASURE_IMAGES ok (${measured} measured, ${skipped} cached, ${failed} failed, ${pruned} pruned; total ${Object.keys(normalized).length})`);
