...
import { initFirebaseAuth, toggleAuth, sendSignInLink, createArchivalFolder, saveToFolder, deleteAccount, currentUser, fetchArchivalFolders } from './modules/auth.js';
...
async function loadArchiveData() {
  ...
  const { archiveData: staticData } = await import('../database.js?v=bad533e');
...
function setupEventListeners() {
  ...
  $('btn-sign-out-account')?.addEventListener('click', () => {
...
function updateMobileSheetUrl() {
  ...
  if (yr.max < 2025) params.set('y_max', yr.max);
  const qs = params.toString();
  // Must mirror syncHashFromState()'s hash form (#grid?...) — that's what
  // handleRouting() parses on load. A /archive?qs query string is ignored by
  // the router and restores no filters, so emit the restorable hash URL.
