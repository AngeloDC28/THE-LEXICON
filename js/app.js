...
async function loadArchiveData() {
  ...
  const { archiveData: staticData } = await import('../database.js?v=a027867');
...
async function applyLanguage(code) {
  ...
  showToast((ALL_LANGS.find(l => l.code === code)?.label || code.toUpperCase()) + ' →');
...
function renderLangDropdown() {
  ...
  dd.innerHTML = ALL_LANGS.map(({ code, label }) => {
    ...
    return `<button type="button" role="option" aria-selected="${isActive}" data-lang-code="${code}" class="w-full text-left px-3 py-2 min-h-[44px] text-[var(--t-mono-xs)] font-mono tracking-widest ${activeClass} transition-colors flex items-center justify-between gap-3 focus-ring">
      <span>${label}</span> <span class="opacity-50 text-[8px] uppercase">${code.toUpperCase()}</span>
    </button>`;
  }).join('');
