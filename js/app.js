...
async function loadArchiveData() {
  ...
  const { archiveData: staticData } = await import('../database.js?v=fa26690');
...
function setupEventListeners() {
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if ((e.key === 'Enter' || e.key === ' ') && document.activeElement?.classList.contains('grid-cell')) {
      ...
      const idx = parseInt(document.activeElement.dataset.imgIndex) || 0;
      if (id) openDetail(id, idx, archiveData, callbacks);
    }
  ...
  // Suppress while the welcome modal is open; cookie banner reveals on dismissal.
  var cookieChoice;
  try { cookieChoice = localStorage.getItem('lexicon-terms-accepted'); } catch(e) {}
  const welcomeOpen = $('welcome-modal') && !$('welcome-modal').hasAttribute('hidden');
  if (!cookieChoice && !welcomeOpen) {
    $('cookie-banner')?.classList.remove('hidden');
  } else if (!cookieChoice && welcomeOpen) {
    // Reveal banner when the welcome modal closes
    const modal = $('welcome-modal');
    const reveal = () => {
      if (modal.hasAttribute('hidden')) $('cookie-banner')?.classList.remove('hidden');
    };
